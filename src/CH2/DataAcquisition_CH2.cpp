...
void acquire_data(Channel_CH2 &channel, rp_channel_t rp_channel)
{
    try
    {
        ...
        while (!stop_acquisition.load())
        {
            ...
            if (rp_AcqAxiGetWritePointer(rp_channel, &pwrite) == RP_OK)
            {
                ...
                if (distance >= samples_per_chunk)
                {
                    ...
                    auto part = std::make_shared<data_part_CH2_t>();
                    convert_raw_data_CH2(buffer_raw, part->data, samples_per_chunk);
